...
var playerFirst = new Student("Dima", "Petrov", 1985, Array.from({ length: subjects }, function () { return Math.floor((Math.random() * diff) + min_mark); }));
var arr = Array.from({ length: lessons }, function () { return (Math.random() > 0.1); });
...
playerFirst.summary();
var playerSecond = new Student("Sergey", "Ivanov", 1991, Array.from({ length: subjects }, function () { return Math.floor((Math.random() * diff) + min_mark); }));
arr = Array.from({ length: lessons }, function () { return (Math.random() > 0.1); });
for (var x = 0; x < arr.length; x++) {
    if (arr[x] === true) {
...
var playerThird = new Student("Andriy", "Shevchnko", 1977, Array.from({ length: subjects }, function () { return Math.floor((Math.random() * diff) + min_mark); }));
arr = Array.from({ length: lessons }, function () { return (Math.random() > 0.1); });
for (var x = 0; x < arr.length; x++) {
    if (arr[x] === true) {
        playerThird.present();
